            ...
            <div key={bucket.name} className={`rounded-2xl border p-8 ${bucket.color} transition-all hover:shadow-lg`}>
              ...
              <div className="space-y-4">
                {bucket.events.map((event, eventIdx) => (
                  <div
                    key={event.id}
                    className="group p-4 rounded-xl bg-white/50 dark:bg-white/5 hover:bg-white dark:hover:bg-white/10 transition-all cursor-pointer border border-transparent hover:border-primary/30"
                  >
                    {/* Event Title */}
              ...
              {/* View All Button */}
              <button className="w-full mt-6 px-4 py-3 border-2 border-border hover:border-primary text-foreground hover:text-primary rounded-lg font-semibold transition-all">
